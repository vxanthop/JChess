...
const scoreSheet = document.getElementById("my-table");
...
async function renderPromotionMenu(x, y) {
  let validOptions = {
    queen: "Queen",
    bishop: "Bishop",
    knight: "Knight",
  ...
  const input = await Swal.fire({
    title: "Select the piece you wish to promote to",
    input: "radio",
    inputOptions: validOptions,
    inputValue: "queen",
  ...
  return pieceSelector[color + input.value];
}
...
function sameDestination(move1) {
  return function (move2) {
    return move1.toX == move2.toX && move1.toY == move2.toY;
  };
}
...
function moveDisambiguation(move) {
    ...
    case "rook":
      for (let x = 0; x < ROWS; x++) {
        for (let y = 0; y < COLS; y++) {
          if (
            oldBoard[y][x] == move.piece &&
            (move.fromX != x || move.fromY != y)
          )
  ...
  for (let i = 0; i < moves.length; ++i) {
    if (moves[i].fromX == move.fromX) {
      disambiguationString += squareToAlgebraic[move.fromY][move.fromX].charAt(
        1
      );
      break;
    }
